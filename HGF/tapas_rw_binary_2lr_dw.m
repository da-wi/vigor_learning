function [traj, infStates] = tapas_rw_binary_2lr_dw(r, p, varargin)
...
if ~isempty(varargin) && strcmp(varargin{1},'trans');
    p = tapas_rw_binary_2lr_dw_transp(r, p);
end
...
for k = 2:1:n
    if not(ismember(k-1, r.ign))
        ...
        if cue_value(k-1) > 0
            % gain trials
            avg_reward(k) = avg_reward(k-1) + alr*(outcome(k-1)-avg_reward(k-1));
            avg_punish(k) = avg_punish(k-1);
        elseif cue_value(k-1) < 0
            % lose trials
            avg_reward(k) = avg_reward(k-1);
            avg_punish(k) = avg_punish(k-1) + all*(abs(outcome(k-1))-avg_punish(k-1));
        elseif cue_value(k-1) == 0
            % neutral trials
            avg_reward(k) = avg_reward(k-1);
            avg_punish(k) = avg_punish(k-1);
        end
        avg_outcome(k) = avg_outcome(k-1) + mean([all alr])*(outcome(k-1)-avg_outcome(k-1));
        ...
        elseif cue_value(k) > 0
            % gain trials
            pGain(k) = pGain(k-1) + alr * (pe(k)/abs(cue_value(k)));            
        elseif cue_value(k) < 0
            % gain trials
            pGain(k) = pGain(k-1) + all * (pe(k)/abs(cue_value(k)));    
        end
